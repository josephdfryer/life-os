...
const DEFAULT_WORKSPACE =
  process.env.THEORY_WORKSPACE_ID ?? "default-workspace";
...
export async function getTheorySourcesForPerson(
  ...
  const taggedNotes = await db.note.findMany({
    where: {
      ...
      OR: [
        ...
        { metadata: { contains: personId, mode: "insensitive" as const } },
      ],
    },
    select: { id: true },
    orderBy: { timestamp: "desc" },
  ...
  const evidenceClaims = await db.evidenceClaim.findMany({
    where: {
      ...
      subjects: {
        some: {
          relevanceWeight: { gt: 0 },
          mention: {
            resolvedPersonId: personId,
            resolutionStatus: { in: ["resolved", "corrected"] },
          },
        },
      },
    },
    ...
    include: {
      ...
      subjects: {
        include: {
          mention: {
            select: { resolvedPersonId: true, sourceText: true, role: true },
          },
        },
      },
    },
  ...
  const noteIds = dedupe([
    ...taggedNotes.map((n) => n.id),
    ...provenanceNoteIds,
  ]);
  ...
  const sources: TheorySource[] = [
    ...
    ...evidenceClaims.map((claim) => {
      const direct = claim.subjects.filter(
        (subject) => subject.mention.resolvedPersonId === personId,
      );
      const participants = claim.subjects
        .map(
          (subject) => `${subject.mention.sourceText} (${subject.subjectRole})`,
        )
        .join(", ");
      return {
        ...
        weight: Math.max(
          ...direct.map((subject) => subject.relevanceWeight),
          0,
        ),
