...
const migrationName = "20260724040000_add_inventory_procurement"

// Idempotent production apply for Phase 3 inventory procurement (suppliers,
// purchase orders, receiving). Mirrors
// prisma/migrations/20260724040000_add_inventory_procurement. Additive only:
// new SupplierProfile/PurchaseOrder/PurchaseOrderLine/PurchaseReceiptLine
// tables plus indexes. Safe to re-run. Depends on Phase 2 (ItemDefinition,
// InventoryLot) already being applied.
...
async function main() {
  ...
  try {
    await client.batch([
      ...
      `CREATE TABLE IF NOT EXISTS "PurchaseReceiptLine" (
        "id" TEXT NOT NULL PRIMARY KEY,
        "workspaceId" TEXT NOT NULL,
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS "SupplierProfile_groupId_key" ON "SupplierProfile"("groupId")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "SupplierProfile_workspaceId_supplierCode_key" ON "SupplierProfile"("workspaceId", "supplierCode")`,
      `CREATE INDEX IF NOT EXISTS "SupplierProfile_workspaceId_active_idx" ON "SupplierProfile"("workspaceId", "active")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "PurchaseOrder_planId_key" ON "PurchaseOrder"("planId")`,
      `CREATE UNIQUE INDEX IF NOT EXISTS "PurchaseOrder_workspaceId_orderNumber_key" ON "PurchaseOrder"("workspaceId", "orderNumber")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseOrder_workspaceId_orderedAt_idx" ON "PurchaseOrder"("workspaceId", "orderedAt")`,
      ...
      `CREATE INDEX IF NOT EXISTS "PurchaseOrderLine_definitionId_idx" ON "PurchaseOrderLine"("definitionId")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseOrderLine_destinationPlaceId_idx" ON "PurchaseOrderLine"("destinationPlaceId")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseReceiptLine_workspaceId_createdAt_idx" ON "PurchaseReceiptLine"("workspaceId", "createdAt")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseReceiptLine_eventId_idx" ON "PurchaseReceiptLine"("eventId")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseReceiptLine_orderLineId_idx" ON "PurchaseReceiptLine"("orderLineId")`,
      `CREATE INDEX IF NOT EXISTS "PurchaseReceiptLine_itemId_idx" ON "PurchaseReceiptLine"("itemId")`,
    ], "write")
