...
export function decodeCursor(cursor: string): { timestamp: Date; id: string } {
  ...
  if (Number.isNaN(timestamp.getTime()) || !id) throw new InteractionStreamError("cursor is malformed")
  return { timestamp, id }
}
...
function parseDate(value: string | null | undefined, field: string): Date | undefined {
  ...
  if (Number.isNaN(date.getTime())) throw new InteractionStreamError(`${field} is not a valid date`)
  return date
}
...
type StreamRow = { timestamp: Date; id: string; amount?: number | null } & Record<string, unknown>
...
export async function aggregateInteractions(
  ...
  if (!grouping) {
    throw new InteractionStreamError(`groupBy must be one of: ${Object.keys(GROUP_BY).join(", ")}`)
  }
  const metric = params.metric ?? "sum"
  if (!METRICS.has(metric)) {
    throw new InteractionStreamError(`metric must be one of: ${[...METRICS].join(", ")}`)
  }
