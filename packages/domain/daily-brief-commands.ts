...
export async function findPersonByName(name: string, workspaceId: string) {
  ...
  return db.person.findFirst({
    where: { workspaceId, first: { equals: first }, ...(last ? { last: { equals: last } } : {}) },
    select: { id: true },
  })
}
...
export async function completeActionItemByText(description: string, workspaceId: string) {
  const { db } = await import("@life-os/db")
  const needle = description.toLowerCase().slice(0, 40)
  const interactions = await db.interaction.findMany({
    where: { workspaceId, actionItems: { contains: description.slice(0, 50) } },
  ...
  for (const interaction of interactions) {
    ...
    if (changed) {
      await db.interaction.update({ where: { id: interaction.id }, data: { actionItems: JSON.stringify(next) } })
      updated++
    }
  }
  return { updated }
...
export async function appendPersonNote(personId: string, noteText: string, workspaceId: string) {
  const { db } = await import("@life-os/db")
  const person = await db.person.findFirst({ where: { id: personId, workspaceId }, select: { notes: true } })
...
registerReviewCommand("daily_brief.complete_action_item", async (input, ctx: ReviewItemActorContext): Promise<ReviewItemCommandResult> => {
  const result = await completeActionItemByText(input.description as string, ctx.workspaceId)
  return { resultType: "Interaction", resultId: result.updated > 0 ? "updated" : null }
})

registerReviewCommand("daily_brief.create_follow_up_plan", async (input, ctx: ReviewItemActorContext): Promise<ReviewItemCommandResult> => {
  const plan = await createPlan(
    { text: input.text as string, personId: (input.personId as string | undefined) ?? undefined },
    ctx.workspaceId,
    ctx.actor,
  )
...
registerReviewCommand("daily_brief.append_person_note", async (input, ctx: ReviewItemActorContext): Promise<ReviewItemCommandResult> => {
  const person = await appendPersonNote(input.personId as string, input.note as string, ctx.workspaceId)
