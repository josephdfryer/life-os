...
export function registerOrganizationReviewHandlers() {
  ...
  registerReviewCommand("attributeMerchantToOrganization", async (input, context) => {
    const { db } = await import("@life-os/db")
    const groupId = String(input.groupId ?? "")
    const merchant = String(input.merchant ?? "").trim()
    if (!groupId || !merchant) throw new Error("A merchant and organization are required")
    ...
    const matches = await db.interaction.findMany({
      where: { workspaceId: context.workspaceId, type: "financial", summary: { contains: merchant } },
      select: { id: true },
    })
    for (const match of matches) {
      await setInteractionGroup(match.id, group.id, context.workspaceId, context.actor)
    }
    ...
    await db.rule.create({
      data: {
        ...
        mode: "suggest",
        status: "active",
        conditions: JSON.stringify([{ field: "summary", operator: "contains", value: merchant }]),
        actions: JSON.stringify([{ type: "set_interaction_group", value: group.id }]),
 ...
 */
export async function merchantAttributionBlastRadius(merchant: string, workspaceId = "default-workspace") {
  const { db } = await import("@life-os/db")
  const trimmed = merchant.trim()
  if (!trimmed) return { interactions: 0, totalAmount: 0, alreadyAttributed: 0 }
  const [all, attributed] = await Promise.all([
    db.interaction.aggregate({
      where: { workspaceId, type: "financial", summary: { contains: trimmed } },
      _count: { _all: true },
      _sum: { amount: true },
    }),
    db.interaction.count({
      where: { workspaceId, type: "financial", summary: { contains: trimmed }, groupId: { not: null } },
    }),
  ])
  return {
