...
export function appendUniqueLine(
...
async function writeInteractionParticipants(
  ...
  if (fields.personId)
    rows.push({ entityType: "Person", entityId: fields.personId });
  if (fields.eventId)
    rows.push({ entityType: "Event", entityId: fields.eventId });
...
export async function appendDailySourceInteraction(
  ...
  return db.$transaction((tx) => appendDailySourceInteractionTx(tx, input), {
    timeout: 20_000,
    maxWait: 10_000,
  });
}
...
export async function appendDailySourceInteractionTx(
  ...
  const existingSource = await findInteractionByExactSource(
    tx,
    input.source,
    input.sourceId,
    input.personId,
  ...
  if (existingSource)
    return { interactionId: existingSource.id, created: false, updated: false };
  ...
  const line = messageLine(
    input.timestamp,
    input.direction,
    input.body || input.summary || "(no text)",
  );
  ...
  if (input.eventId) {
    ...
    if (!event)
      throw new Error("The source Event does not exist in this workspace");
    ...
    await writeInteractionParticipants(
      tx,
      interaction.id,
      { personId: input.personId, eventId: event.id },
      workspaceId,
    );
  ...
  const dailyInteraction = await tx.interaction.findFirst({
    where: {
      ...
      workspaceId,
      type: input.type,
      notes: { contains: dayMarker, mode: "insensitive" as const },
    },
  ...
  if (dailyInteraction) {
    const interaction = await tx.interaction.update({
      ...
      data: {
        ...
        direction:
          dailyInteraction.direction === input.direction
            ? dailyInteraction.direction
            : "mixed",
      },
      select: { id: true },
    });
  ...
  await writeInteractionParticipants(
    tx,
    interaction.id,
    { personId: input.personId, eventId: event?.id ?? null },
    workspaceId,
  );
  await publishGraphEvent(tx, {
    workspaceId,
...
export async function acceptStagedInteraction(
  ...
  const { db } = await import("@life-os/db");
  const workspaceId =
    input.workspaceId ?? input.actor?.workspaceId ?? "default-workspace";

  const item = await db.stagedInteraction.findFirst({
    where: { id: input.id, workspaceId },
  });
  if (!item)
    throw new AcceptStagedInteractionError(
      "Staged interaction not found",
      "not_found",
    );
  ...
  if (item.status === "accepted") {
    if (!item.interactionId) {
      throw new AcceptStagedInteractionError(
        "Item is marked accepted but has no interactionId",
        "invalid_state",
      );
    }
    return {
  ...
  if (item.status !== "pending" && item.status !== "blocked") {
    throw new AcceptStagedInteractionError(
      `Cannot accept an item with status "${item.status}"`,
      "invalid_state",
    );
  }
  ...
  if (!personId)
    throw new AcceptStagedInteractionError(
      "Choose a Person before accepting this item",
      "validation",
    );
...
function parseSourceMetadata(raw: string | null | undefined): {
  eventId?: string;
} {
  try {
