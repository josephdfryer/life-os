...
const WORKSPACE_ID = process.env.PHOTOS_SYNC_WORKSPACE_ID ?? "default-workspace"
...
async function main() {
  ...
  if (dryRun) {
    for (const [day, dayPhotos] of byDay) {
      console.log(`  ${day}: ${dayPhotos.length} photo(s), people=${uniquePersons(dayPhotos).join(", ") || "none"}`)
    }
...
async function writeDayDigest(day: string, photos: OsxPhoto[]) {
  ...
  const captions = [...new Set(photos.map(p => p.ai_caption).filter((c): c is string => Boolean(c)))].slice(0, MAX_CAPTIONS_PER_DAY)
  ...
  if (existing) {
    await db.note.update({ where: { id: existing.id }, data: { content, metadata } })
  } else {
...
async function findNoteByMarker(marker: string) {
  const candidates = await db.note.findMany({
    where: { workspaceId: WORKSPACE_ID, type: "photo_digest", metadata: { contains: marker } },
