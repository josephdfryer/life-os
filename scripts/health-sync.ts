...
const DEFAULT_PERSON_ID =
  process.env.HEALTH_SYNC_PERSON_ID ?? "cmr9gi8eb008104lek5io7qoa";
...
async function main() {
  ...
  if (!zipPath) {
    throw new Error(
      `No HealthAutoExport_*.zip found in ${options.zipDir}. Pass --zip <path> explicitly.`,
    );
  }
  ...
  console.log(
    `[health-sync] Resolved self Person: ${person.first} ${person.last} (${person.id}, workspace ${person.workspaceId})`,
  );
  ...
  if (!options.skipDaily) {
    const dailyRows = options.limitDays
      ? parsed.dailyRows.slice(-options.limitDays)
      : parsed.dailyRows;
    ...
    const runNote = await db.note.create({
      data: {
        ...
        metadata: JSON.stringify({
          sourceMarker: `${SOURCE}:workout-run:${parsed.zipFilename}:${Date.now()}`,
        }),
      },
      select: { id: true },
    });
...
function reportParse(parsed: ParsedExport) {
  ...
  if (parsed.sameMinuteCollisions.length > 0) {
    console.log(
      `[health-sync] WARNING: ${parsed.sameMinuteCollisions.length} same-type-same-minute workout collision(s) detected — verify these are distinct workouts, not near-duplicates:`,
    );
...
async function ensureStateDefinitions(
  workspaceId: string,
...
async function syncDailyRow(input: {
  ...
  const digest = metrics.map((m) => `${m.key}=${m.value}`).join(" ");
  ...
  const recorded = await db.$transaction((tx) =>
    replaceStatesForSourceNoteInTransaction(
      ...
      metrics.map((metric) => {
        ...
        if (!definitionId)
          throw new Error(
            `No StateDefinition resolved for metric key "${metric.key}"`,
          );
        return {
          entityType: WORKSPACE_ENTITY_TYPE,
          entityId,
  ...
  if (input.verbose)
    console.log(`[health-sync] ${row.dayKey}: ${metrics.length} metric(s)`);
...
async function syncWorkout(input: {
  ...
  if (input.verbose)
    console.log(
      `[health-sync] workout ${workout.sourceId}: ${created ? "created" : "updated"}${routeArchived ? " (+route)" : ""}`,
    );
...
function summarizeWorkout(workout: WorkoutRow): string {
  ...
  if (workout.distanceMiles)
    parts.push(`${workout.distanceMiles.toFixed(2)}mi`);
  parts.push(`${Math.round(workout.durationMinutes)}min`);
  if (workout.avgHeartRate)
    parts.push(`avg HR ${Math.round(workout.avgHeartRate)}bpm`);
  if (workout.activeEnergyKcal)
    parts.push(`${Math.round(workout.activeEnergyKcal)} kcal active`);
...
function parseArgs(args: string[]): Options {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === "--zip-dir" && next)
      ((options.zipDir = expandHome(next)), i++);
    else if (arg === "--dry-run") options.dryRun = true;
    else if (arg === "--person-id" && next) ((options.personId = next), i++);
    else if (arg === "--limit-days" && next)
      ((options.limitDays = Number(next)), i++);
