...
export async function resolveExtraction(
  ...
  const resolvedPlaceIds = await resolvePlaces(
    extraction.places_mentioned ?? [],
    db,
  );
  const resolvedPlanIds = await resolvePlans(
    extraction.plans_mentioned ?? [],
    db,
  );
...
async function findPerson(
  ...
  if (participant.email) {
    const persons = await db.person.findMany({
      where: {
        ...
        emails: {
          contains: participant.email.toLowerCase(),
  ...
  if (participant.phone) {
    ...
    if (normalized) {
      ...
      const match = loose.find((p) =>
        safeJsonArray(p.phones).some((phone) =>
          phoneNumbersMatch(phone, participant.phone!),
        ),
      );
...
async function resolvePlans(
  ...
  for (const mention of mentions) {
    const plan = await db.plan.findFirst({
      where: {
        workspaceId: WORKSPACE_ID,
        text: { contains: mention, mode: "insensitive" as const },
      },
      select: { id: true },
    });
