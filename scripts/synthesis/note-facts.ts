...
const EXTRACTABLE_TYPES = ["document", "voice_transcript", "photo_digest"] as const
...
async function main() {
  ...
  const limit = Number(process.argv.find((_, i, arr) => arr[i - 1] === "--limit")) || 100
  ...
  const notes = await db.note.findMany({
    where: {
      ...
      NOT: { metadata: { contains: '"extraction":' } },
    },
  ...
  for (let index = 0; index < notes.length; index += BATCH_SIZE) {
    ...
    const results = await Promise.all(batch.map(async note => {
      try {
        ...
        const facts = await callClaudeForFacts(note.type, note.content.slice(0, cap))
        if (!facts) return "skipped" as const

        if (dryRun) {
          console.log(`[note-facts] ${note.id} (${note.type}): ${facts.summary}`)
          console.log(`  facts: ${facts.facts.map(f => `${f.key}=${f.value}`).join(", ") || "(none)"}`)
        ...
        console.error(`[note-facts] ${note.id} failed:`, error instanceof Error ? error.message : error)
  ...
  console.log(`[note-facts] Done. extracted=${extracted} skipped=${skipped}${dryRun ? " (--dry-run, nothing written)" : ""}`)
  await db.$disconnect()
}

async function callClaudeForFacts(noteType: string, content: string): Promise<NoteFacts | null> {
  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) throw new Error("Note fact extraction requires ANTHROPIC_API_KEY")
  ...
  const message = await anthropicClient.messages.create({
    model: MODEL,
    max_tokens: 500,
    system: "You extract compact key facts from captured personal notes. Return only valid JSON.",
    messages: [{
      role: "user",
      content: JSON.stringify({
        ...
        responseShape: {
          ...
          facts: "array of up to 8 {key, value} pairs — dates, amounts, names, anything a person would want to look up later",
          entities: "array of up to 6 named people/places/organizations mentioned",
        },
      }),
    }],
  })

  return parseNoteFacts(message.content
...
function parseNoteFacts(text: string): NoteFacts | null {
  try {
    ...
    const facts = Array.isArray(json.facts)
      ? json.facts
        .filter((f): f is Record<string, unknown> => isRecord(f) && typeof f.key === "string" && typeof f.value === "string")
        .slice(0, 8)
        .map(f => ({ key: String(f.key).slice(0, 60), value: String(f.value).slice(0, 200) }))
      : []

    const entities = Array.isArray(json.entities)
      ? json.entities.filter((e): e is string => typeof e === "string").slice(0, 6).map(e => e.slice(0, 100))
      : []
