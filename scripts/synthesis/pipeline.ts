...
async function main() {
  ...
  console.log(`[synthesis] Starting pipeline for date range: ${options.since.toISOString()} → ${options.until.toISOString()}`)
  ...
  console.log(`[synthesis] Triaged: ${worthCount}/${items.length} worth extracting`)
...
async function collectItems(since: Date, until: Date): Promise<RawItem[]> {
  ...
  for (const source of ["whatsapp", "calls"] as const) {
    ...
    for (const file of files) {
      ...
      try {
        const raw = JSON.parse(fs.readFileSync(path.join(sourceDir, file), "utf8")) as unknown[]
        for (const record of raw) {
          const item = archiveRecordToRawItem(source, record, path.join(sourceDir, file))
...
async function collectDbItems(since: Date, until: Date): Promise<RawItem[]> {
  const interactions = await db.interaction.findMany({
    ...
    select: {
      ...
      notes: true,
      duration: true,
      person: { select: { first: true, last: true, emails: true, phones: true } },
    },
  ...
  return interactions.map(interaction => {
    ...
    return {
      ...
      durationSeconds: interaction.duration ? interaction.duration * 60 : undefined,
...
function archiveRecordToRawItem(
  ...
  if (source === "whatsapp") {
    ...
    return {
      ...
      timestamp,
      body: text,
      participants: [{ name: String(r.partnerName ?? ""), identifier: String(r.fromJid ?? r.chatJid ?? id) }],
      archivePath,
    }
  }
  ...
  if (source === "calls") {
    ...
    return {
      ...
      participants: [{ name: String(r.displayName ?? ""), identifier: address }],
...
function parseArgs(args: string[]): Options {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i]
    const next = args[i + 1]
    if (arg === "--days" && next) { days = Number(next); i++ }
    else if (arg === "--since" && next) { since = new Date(next); i++ }
    else if (arg === "--until" && next) { until = new Date(next); i++ }
  }
