...
import { createCalendarAuthorizeUrl } from "@/lib/calendar-oauth"
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!userId) return errorResponse(400, "validation", "No user found to own the Calendar connection")

    const returnTo = req.nextUrl.searchParams.get("returnTo") || "/admin/connections"
    const { url } = createCalendarAuthorizeUrl(auth.workspaceId, userId, returnTo)
