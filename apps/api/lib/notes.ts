...
export async function listNotes(params: ListNotesParams, workspaceId: string) {
  ...
  if (q && q.length > 500)
    throw new NotesApiError("q must be 500 characters or fewer", "validation");
  const type = params.type ? noteTypeContract.safeParse(params.type) : null;
  if (params.type && type && !type.success)
    throw new NotesApiError("type is invalid", "validation");
  ...
  if (params.cursor) {
    ...
    and.push({
      OR: [{ timestamp: { lt: timestamp } }, { timestamp, id: { lt: id } }],
    });
  }
  ...
  return notesPageContract.parse({
    ...
    nextCursor:
      hasMore && last ? encodeNotesCursor(last.timestamp, last.id) : null,
    hasMore,
    limit,
  });
}
...
function boundedLimit(value: number | undefined) {
  if (value !== undefined && !Number.isFinite(value))
    throw new NotesApiError("limit must be a number", "validation");
...
function decodeNotesCursor(cursor: string) {
  ...
  if (separator <= 0)
    throw new NotesApiError("cursor is malformed", "validation");
