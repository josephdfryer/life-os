import { peoplePageContract, personResourceContract, type PersonResource } from "@life-os/contracts"
...
export async function listPeople(params: ListPeopleParams, workspaceId: string) {
  const limit = boundedLimit(params.limit)
  const q = params.q?.trim() || null
  if (q && q.length > 500) throw new PeopleApiError("q must be 500 characters or fewer", "validation")
  ...
  if (params.cursor) {
    ...
    and.push({ OR: [{ createdAt: { gt: createdAt } }, { createdAt, id: { gt: id } }] })
  }
  ...
  return peoplePageContract.parse({
    ...
    nextCursor: hasMore && last ? encodePeopleCursor(last.createdAt, last.id) : null,
    hasMore,
    limit,
  })
}
...
function boundedLimit(value: number | undefined) {
  if (value !== undefined && !Number.isFinite(value)) throw new PeopleApiError("limit must be a number", "validation")
...
function decodePeopleCursor(cursor: string) {
  ...
  if (separator <= 0) throw new PeopleApiError("cursor is malformed", "validation")
  const createdAt = new Date(decoded.slice(0, separator))
  const id = decoded.slice(separator + 1)
  if (Number.isNaN(createdAt.getTime()) || !id) throw new PeopleApiError("cursor is malformed", "validation")
  return { createdAt, id }
}
