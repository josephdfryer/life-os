...
import {
  ...
  recordHealthDailyDigestInTransaction,
  fireHealthDailyRules,
} from "./health-daily";
import {
  OURA_BACKFILL_DAYS,
...
export const OURA_METRIC_TAXONOMY: Record<string, string> = {
  ...
  oura_readiness_activity_balance:
    "Oura readiness contributor: activity balance",
  oura_readiness_body_temperature:
    "Oura readiness contributor: body temperature",
  oura_readiness_hrv_balance: "Oura readiness contributor: HRV balance",
  oura_readiness_previous_day_activity:
    "Oura readiness contributor: previous day activity",
  ...
  oura_readiness_resting_heart_rate:
    "Oura readiness contributor: resting heart rate",
  ...
  oura_activity_meet_daily_targets:
    "Oura activity contributor: meet daily targets",
  ...
  oura_activity_training_frequency:
    "Oura activity contributor: training frequency",
...
const STRESS_SUMMARY: Record<string, number> = {
  restored: 1,
  normal: 2,
  stressful: 3,
};
const WEBHOOK_DATA_TYPES = [
  "daily_readiness",
  "daily_sleep",
  "daily_activity",
  "daily_stress",
...
export function normalizeOuraDay(documents: OuraDayDocuments) {
  ...
  if (stress) {
    ...
    const summary = stress.day_summary
      ? STRESS_SUMMARY[stress.day_summary]
      : undefined;
    if (summary !== undefined)
      samples.push({ key: "oura_stress_summary", value: summary });
  }
...
export async function ingestOuraDay(input: {
  ...
  const definitions =
    input.definitions ??
    (await ensureOuraMetricDefinitions(
      input.workspaceId,
  ...
  const result = await withSqliteBusyRetry(() =>
    db.$transaction(
      (tx) =>
        recordHealthDailyDigestInTransaction(tx, definitions, {
...
export async function backfillOuraConnection(
  connection: OuraConnectionRow,
  days = OURA_BACKFILL_DAYS,
) {
  ...
  for (const collection of OURA_DAILY_COLLECTIONS) {
    try {
      const documents = await fetchOuraCollection(
        accessToken,
        collection,
        start,
        end,
  ...
  if (!personId)
    throw new OuraError(
      "Workspace owner has no linked Person to record Oura States against",
      "validation",
    );

  const keys = [...byDay.values()].flatMap((documents) =>
    normalizeOuraDay(documents).samples.map((sample) => sample.key),
  );
  const definitions = await ensureOuraMetricDefinitions(
  ...
  for (const [day, documents] of [...byDay.entries()].sort(([a], [b]) =>
    b.localeCompare(a),
  )) {
...
export async function ingestOuraWebhookDocument(
  connection: OuraConnectionRow,
  dataType: string,
  objectId: string,
) {
  if (!isDailyCollection(dataType))
    return { skipped: true as const, reason: "unsupported_data_type" };
  const accessToken = await accessTokenFor(connection);
  ...
  const written = await ingestOuraDay({
    workspaceId: connection.workspaceId,
    personId,
    day,
    documents,
  ...
  await markConnectionSynced(connection.id, {
    lastWebhookDay: day,
    lastWebhookDataType: dataType,
  });
  return { skipped: false as const, day, ...written };
}

async function fetchOuraDay(
  accessToken: string,
  day: string,
): Promise<OuraDayDocuments> {
...
export async function ensureOuraWebhookSubscriptions() {
  ...
  const have = new Set(
    existing.map((row) => `${row.data_type}:${row.event_type}`),
  );
...
async function accessTokenFor(connection: OuraConnectionRow) {
  ...
  if (!access || !refresh)
    throw new OuraError("Oura connection has no stored tokens", "revoked");
  const expiring =
  ...
  try {
    ...
    const code =
      error instanceof OuraError && error.code === "revoked"
        ? "revoked"
        : "refresh_failed";
    await db.connection.update({
      where: { id: connection.id },
      data: {
        status: code === "revoked" ? "disabled" : "active",
        lastError: code,
      },
    });
    throw error;
...
export function importedOuraDaysFromNotes(
  notes: { metadata: string | null }[],
) {
  const days = new Set<string>();
  for (const note of notes) {
    const marker = parseMetadata(note.metadata).sourceMarker;
    if (typeof marker === "string" && marker.startsWith("oura:day:"))
      days.add(marker.slice("oura:day:".length));
  }
  return days;
}
...
async function importedOuraDays(workspaceId: string) {
  ...
  const notes = await db.note.findMany({
    where: {
      workspaceId,
      type: "import",
      metadata: { contains: "oura:day:", mode: "insensitive" as const },
    },
    select: { metadata: true },
  });
...
async function ensureOuraMetricDefinitions(
  workspaceId: string,
  keys: Iterable<string>,
) {
  const definitions = new Map<string, string>();
  for (const key of new Set(keys)) {
    const definition = await withSqliteBusyRetry(() =>
      ensureStateDefinition(
        {
          entityType: "Person",
          type: "health_metric",
          value: key,
          description: OURA_METRIC_TAXONOMY[key] ?? null,
        },
        workspaceId,
      ),
    );
...
export function isSqliteBusy(error: unknown) {
  ...
  return /SQLITE_BUSY|database is locked|P2034|P1008/i.test(
    `${code} ${message}`,
  );
}
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, 80 * 2 ** attempt + Math.floor(Math.random() * 40)),
      );
    }
  }
  throw last;
