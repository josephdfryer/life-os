import { plansPageContract, planResourceContract, planStatusContract, type PlanResource } from "@life-os/contracts"
...
export async function listPlans(params: ListPlansParams, workspaceId: string) {
  ...
  if (q && q.length > 500) throw new PlansApiError("q must be 500 characters or fewer", "validation")
  const status = params.status ? planStatusContract.safeParse(params.status) : null
  if (status && !status.success) throw new PlansApiError("status is invalid", "validation")

  const and: Record<string, unknown>[] = []
  if (q) and.push({ text: { contains: q } })
  if (params.cursor) {
  ...
  return plansPageContract.parse({
    ...
    nextCursor: hasMore && last ? encodePlansCursor(last.createdAt, last.id) : null,
    hasMore,
    limit,
  })
}
...
function boundedLimit(value: number | undefined) {
  if (value !== undefined && !Number.isFinite(value)) throw new PlansApiError("limit must be a number", "validation")
...
function storedStringList(raw: string | null): string[] {
  ...
  try {
    ...
    return parsed.filter((item): item is string => typeof item === "string").flatMap(splitLegacyValue)
  } catch {
    return []
  }
}
...
function decodePlansCursor(cursor: string) {
  ...
  if (separator <= 0) throw new PlansApiError("cursor is malformed", "validation")
  const createdAt = new Date(decoded.slice(0, separator))
  const id = decoded.slice(separator + 1)
  if (Number.isNaN(createdAt.getTime()) || !id) throw new PlansApiError("cursor is malformed", "validation")
  return { createdAt, id }
}
