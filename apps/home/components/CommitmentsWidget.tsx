...
export default async function CommitmentsWidget({ workspaceId, personsUrl }: Props) {
  ...
  // Dated and undated are fetched separately so a large backlog of undated
  // commitments can never crowd out what is actually due now.
  const [dated, undated, interactions, clearedThisWeek] = await Promise.all([
