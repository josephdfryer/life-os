...
export async function GET(request: NextRequest) {
  ...
  if (denied) return NextResponse.redirect(new URL("/admin/connections?oura=denied", request.url))

  const code = request.nextUrl.searchParams.get("code")?.trim() ?? ""
  const state = request.nextUrl.searchParams.get("state")?.trim() ?? ""
  const scope = request.nextUrl.searchParams.get("scope")?.trim() ?? "daily"
  if (!code || !state) return NextResponse.redirect(new URL("/admin/connections?oura=invalid", request.url))
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.redirect(new URL(`/admin/connections?oura=${codeName}`, request.url))
    }
    if (body.backfillError) {
      return NextResponse.redirect(new URL("/admin/connections?oura=connected_sync_failed", request.url))
    }
    return NextResponse.redirect(new URL("/admin/connections?oura=connected", request.url))
  } catch {
