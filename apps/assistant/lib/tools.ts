...
async function createItemTool(input: Record<string, unknown>, workspaceId: string) {
  ...
  const item = await createItem({
    ...
    ...(optionalString(input.make) ? { make: optionalString(input.make)! } : {}),
    ...(optionalString(input.model) ? { model: optionalString(input.model)! } : {}),
    ...(optionalString(input.serialNumber) ? { serialNumber: optionalString(input.serialNumber)! } : {}),
    ...(optionalString(input.notes) ? { notes: optionalString(input.notes)! } : {}),
    ...(optionalString(input.placeId) ? { placeId: optionalString(input.placeId)! } : {}),
  }, workspaceId, ASSISTANT_ACTOR)
