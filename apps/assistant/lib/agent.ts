...
function systemPrompt(
  ...
  return [
    ...
    "You can also work Linear on the member's behalf — search_linear_issues, get_linear_issue, create_linear_issue, update_linear_issue (status/assignee/priority), and add_linear_comment. Use these when the member wants to check, file, update, or comment on a Linear ticket. Issue keys look like JF-157; confirm one via search before updating or commenting if the member only described the issue.",
