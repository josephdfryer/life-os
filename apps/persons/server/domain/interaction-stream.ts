...
// calendar, transactions — instead of a page per person. Two endpoints:
// this one walks rows, `aggregate` answers "how much / how many" without
// shipping the rows at all.
//
...
export function decodeCursor(cursor: string): { timestamp: Date; id: string } {
  ...
  if (separator <= 0) throw badRequest("cursor is malformed", { field: "cursor" })
  const timestamp = new Date(decoded.slice(0, separator))
  const id = decoded.slice(separator + 1)
  if (Number.isNaN(timestamp.getTime()) || !id) throw badRequest("cursor is malformed", { field: "cursor" })
  return { timestamp, id }
}
...
function parseDate(value: string | null | undefined, field: string): Date | undefined {
  ...
  if (Number.isNaN(date.getTime())) throw badRequest(`${field} is not a valid date`, { field })
  return date
}
...
function buildWhere(params: StreamParams, workspaceId: string) {
  ...
  if (since || until) {
    and.push({ timestamp: { ...(since ? { gte: since } : {}), ...(until ? { lt: until } : {}) } })
  }
  ...
  if (params.category) and.push({ category: { contains: params.category } })
  if (params.direction) and.push({ direction: params.direction })
  if (params.source) and.push({ source: params.source })

  if (params.minAmount != null) and.push({ amount: { gte: Math.round(params.minAmount) } })
  if (params.maxAmount != null) and.push({ amount: { lte: Math.round(params.maxAmount) } })

  if (params.q) {
    and.push({ OR: [{ merchantName: { contains: params.q } }, { summary: { contains: params.q } }] })
  }
  ...
  if (params.groupId) {
    and.push({
      OR: [
        { actorPerson: { groupMemberships: { some: { groupId: params.groupId } } } },
        { participants: { some: { entityType: "Group", entityId: params.groupId } } },
      ],
    })
  }
...
function buildInclude(include: string | null | undefined) {
  ...
  return {
    ...
    ...(wanted.has("person") ? { person: { select: { id: true, first: true, last: true, company: true } } } : {}),
    ...(wanted.has("place") ? { place: { select: { id: true, name: true, googlePlaceId: true } } } : {}),
    ...(wanted.has("event") ? { event: { select: { id: true, name: true, start: true } } } : {}),
    ...(wanted.has("account") ? { accountLink: { select: { id: true, institution: true, accountName: true, isShared: true } } } : {}),
    ...(wanted.has("actor") ? { actorPerson: { select: { id: true, first: true, last: true } } } : {}),
    ...(wanted.has("participants")
      ? { participants: { select: { entityType: true, entityId: true, role: true, band: true, confidence: true } } }
      : {}),
  }
}

export async function streamInteractions(params: StreamParams, workspaceId: string) {
  ...
  const total = params.withTotal
    ? await db.interaction.count({ where: buildWhere(params, workspaceId) as never })
    : undefined
...
type StreamRow = Awaited<ReturnType<typeof db.interaction.findMany>>[number] & Record<string, unknown>
...
const GROUP_BY: Record<string, { sql: string; join?: string; label: string }> = {
  ...
  direction: { sql: `COALESCE(i."direction", 'unknown')`, label: "direction" },
  category: { sql: `COALESCE(NULLIF(i."category", ''), 'uncategorized')`, label: "category" },
  merchant: { sql: `COALESCE(NULLIF(i."merchantName", ''), i."summary", 'unknown')`, label: "merchant" },
...
export async function aggregateInteractions(
  ...
  if (!grouping) {
    throw badRequest(`groupBy must be one of: ${Object.keys(GROUP_BY).join(", ")}`, { field: "groupBy" })
  }
  const metric = params.metric ?? "sum"
  if (!METRICS.has(metric)) {
    throw badRequest(`metric must be one of: ${[...METRICS].join(", ")}`, { field: "metric" })
  }
  const limit = Math.min(200, Math.max(1, Math.round(params.limit ?? 25)))

  const { clauses, values } = rawFilters(params, workspaceId)
  const aggregate = metric === "count" ? `COUNT(*)` : `${metric.toUpperCase()}(i."amount")`
  ...
  const rows = await db.$queryRawUnsafe<{ bucket: string | null; metric_value: number | null; row_count: number }[]>(
    sql, ...values,
  )

  const [totals] = await db.$queryRawUnsafe<{ total: number | null; n: number }[]>(
  ...
  return {
    ...
    metric,
    total: centsToDollars(totals?.total ?? 0),
    ...
    groups: rows
      ...
      .map(row => ({
        ...
        value: inDollars ? centsToDollars(row.metric_value) : Number(row.metric_value),
        valueCents: inDollars ? Number(row.metric_value ?? 0) : undefined,
...
function rawFilters(params: StreamParams, workspaceId: string) {
  ...
  const types = csv(params.type)
  if (types.length) push(`i."type" IN (${types.map(() => "?").join(",")})`, ...types)
  const subtypes = csv(params.subtype)
  if (subtypes.length) push(`i."subtype" IN (${subtypes.map(() => "?").join(",")})`, ...subtypes)
  ...
  if (params.category) push(`LOWER(COALESCE(i."category", '')) LIKE ?`, `%${params.category.toLowerCase()}%`)
  if (params.minAmount != null) push(`i."amount" >= ?`, Math.round(params.minAmount))
  if (params.maxAmount != null) push(`i."amount" <= ?`, Math.round(params.maxAmount))
  if (params.q) {
    push(
      `(LOWER(COALESCE(i."merchantName", '')) LIKE ? OR LOWER(COALESCE(i."summary", '')) LIKE ?)`,
      `%${params.q.toLowerCase()}%`, `%${params.q.toLowerCase()}%`,
    )
  }
  if (params.groupId) {
    push(
