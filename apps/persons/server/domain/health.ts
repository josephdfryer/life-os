...
export async function getPersonHealthSummary(
  ...
  const latest = await db.state.findFirst({
    where: { workspaceId, entityId: personId, entityType: "Person", source: HEALTH_SOURCE },
  ...
  const [latestDayStates, recentNotes, totalReadings] = await Promise.all([
    ...
    db.note.findMany({
      where: {
        workspaceId,
        type: "import",
        metadata: { contains: `"${HEALTH_SOURCE}:day:` },
      },
  ...
  const metrics: HealthMetricPoint[] = HIGHLIGHT_METRICS
    .map(({ key, label }) => {
      const state = byKey.get(key)
      if (!state || state.severity === null) return null
      return { key, label, value: state.severity, unit: state.definition.description ?? "" }
    })
  ...
  return {
    ...
    metrics,
    recentLog: recentNotes.map(n => ({ id: n.id, date: n.timestamp.toISOString(), content: n.content })),
    totalReadings,
  }
}
