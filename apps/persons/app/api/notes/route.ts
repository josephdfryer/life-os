...
export async function POST(req: NextRequest) {
  try {
    ...
    if (Number.isNaN(timestamp.getTime())) {
      return NextResponse.json(
        { error: { code: "validation", message: "timestamp is invalid" } },
        { status: 400 },
      );
    }
    ...
    const { note } = await captureNote({
      ...
      metadata:
        ...
        !Array.isArray(payload.metadata)
          ...
          : null,
      aboutPersonId:
        typeof payload?.aboutPersonId === "string"
          ? payload.aboutPersonId
          : payload?.personId,
      aboutPlaceId:
        typeof payload?.aboutPlaceId === "string"
          ? payload.aboutPlaceId
          : payload?.placeId,
      aboutItemId:
        typeof payload?.aboutItemId === "string"
          ? payload.aboutItemId
          : payload?.itemId,
      aboutEventId:
        typeof payload?.aboutEventId === "string"
          ? payload.aboutEventId
          : payload?.eventId,
      aboutPlanId:
        typeof payload?.aboutPlanId === "string"
          ? payload.aboutPlanId
          : payload?.planId,
      aboutGroupId:
        typeof payload?.aboutGroupId === "string"
          ? payload.aboutGroupId
          : payload?.groupId,
      aboutStateId:
        typeof payload?.aboutStateId === "string"
          ? payload.aboutStateId
          : payload?.stateId,
    });
