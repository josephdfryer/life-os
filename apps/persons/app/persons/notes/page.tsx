...
export default function NotesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error?.message ?? data.error ?? "Could not load notes")
      setNotes(data.notes ?? [])
  ...
  async function loadMore() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error?.message ?? data.error ?? "Could not load more")
      setNotes(prev => {
  ...
  async function capture() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error?.message ?? data.error ?? "Could not save note")
      setDraft("")
          ...
          {detail && (
            <>
              ...
              <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                {(detail.aboutPersonId
                  || (detail.metadata != null && typeof detail.metadata === "object" && "subjectPersonId" in (detail.metadata as Record<string, unknown>))) && (
                  <Link
                    href={`/persons/${detail.aboutPersonId ?? (detail.metadata as Record<string, string>).subjectPersonId}/theory`}
