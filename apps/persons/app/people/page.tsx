...
export default async function PeoplePage() {
  try {
    ...
    return <PeopleClient initialData={{ persons: persons as never, total, hasMore: LIMIT < total }} />
  } catch (error) {
    if (error instanceof AppError && error.status === 401) {
      redirect("/login?callbackUrl=%2Fpeople")
    }
    return <PeopleClient initialData={null} />
  }
}
