...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    items.map((item) => ({
      ...item,
      purchasePrice: centsToDollars(item.purchasePrice),
    })),
  );
}
...
export async function POST(req: NextRequest) {
  ...
  const item = await createItem(
    {
      ...
      ...(purchasePrice != null && purchasePrice !== ""
        ? { purchasePrice: dollarsToCents(purchasePrice) }
        : {}),
      ...(purchaseFrom ? { purchaseFrom: purchaseFrom as string } : {}),
      ...(warrantyExpires
        ? { warrantyExpires: new Date(warrantyExpires as string) }
        : {}),
      lifetimeWarranty: Boolean(lifetimeWarranty),
      ...(warrantyDetails
        ? { warrantyDetails: warrantyDetails as string }
        : {}),
    },
    workspaceId,
  ...
  return NextResponse.json(
    { ...item, purchasePrice: centsToDollars(item.purchasePrice) },
    { status: 201 },
  );
}
