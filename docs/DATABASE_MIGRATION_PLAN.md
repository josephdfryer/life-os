...
**Status:** execution started 2026-08-28 on branch `codex/postgres-migration`.
Turso reads restored; **P0 complete** (JSON + raw-table + Turso-native
snapshot, all checksummed). **P2–P4 and most of P6 landed and are verified
locally**: schema on `postgresql`, migrations squashed to a 2-file baseline,
client on `@prisma/adapter-pg`, the whole test + e2e harness and the deploy
gates ported off SQLite. Repo-wide `npm run type-check`, full `npm test`
(every workspace + `scripts/**`), and `npm run lint` all pass against a
local Postgres 17. **Remaining before cutover:** provision Neon (**P1 —
blocked on the account owner, critical path**), then run the ETL rehearsals
(P5 program is written but unrun) and the P8 cutover. A P10 cleanup sweep of
~45 non-critical one-off libSQL scripts is deferred. See the ledger below.
...
| P5     | **Validated by a full run** | `scripts/db/migrate-turso-to-postgres.ts` ran Turso → `life-os-postgres` end to end in **4m47s**: 95 models, **217,650 source rows == 217,650 target rows**, every per-model content hash matched, `"valid": true`, `"failures": []`. Report saved to `backups/etl-rehearsal-1-2026-08-28.txt` (gitignored). The guarded "repairs" path fired only for orphaned **test-fixture** Workspaces (ids matching `test|api-`, e.g. `audit-api-a-*`, `device-contacts-test-*`) — synthesized placeholder parents rather than aborting, as designed. Those junk rows are integration-test residue in the Turso prod DB and worth cleaning at source before the real cutover. Insert order is schema-declaration order + `SET CONSTRAINTS ALL DEFERRED`; it worked, so `00000000000001_defer_foreign_keys` covers the FK graph adequately. |
...
| P7     | **Rehearsal 1 clean; rehearsal 2 pending** | Rehearsal 1 = the P5 run above, straight into `life-os-postgres`/`neondb`. Rehearsal 2 (from a freshly reset target, requiring identical results) is pending a decision: resetting that database is a destructive action that needs explicit approval, and it may be moot if the region question moves the project. |
| P8–P10 | Not started                         | P8 (cutover) waits on the region decision, `neon-amber-pillar` cleanup, wiring `DATABASE_URL`/`DATABASE_URL_UNPOOLED` into all 8 Vercel projects, and a rehearsal-2 pass. P9/P10 follow. |
