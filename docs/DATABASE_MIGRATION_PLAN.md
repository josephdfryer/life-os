...
| P5     | **Program written, unrun** (`93d57bd`) | `scripts/db/migrate-turso-to-postgres.ts` (419 lines): parses models from the frozen SQLite schema, keyset/offset batched copy inside one transaction with `SET CONSTRAINTS ALL DEFERRED`, source-vs-target content-hash + row-count reconciliation, and a guarded "repairs" path for orphaned test-fixture Workspace/Note parents (aborts on any non-test orphan). Review notes before first run: insert order is schema-declaration order and leans entirely on `00000000000001_defer_foreign_keys` — confirm it covers **every** FK, not most; the Note-repair path sets `metadata`/`content` as `JSON.stringify(...)` strings, which a `jsonb` column would reject (all these columns are currently `String`, so fine today — recheck if any become `Json`). Add `default-workspace` handling: `prisma migrate deploy` on the target does **not** seed it (the seed lives in test/e2e helpers only, to keep the target empty for the ETL), so the real row copies from Turso like any other — no conflict. |
...
| P7–P10 | Not started                         | P7 (ETL rehearsals) and P8 (cutover) wait on P1 (Neon). P9/P10 follow. |
