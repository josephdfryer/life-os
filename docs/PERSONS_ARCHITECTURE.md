...
New People start at the lowest closeness level (Acquaintance), which has no
ambient follow-up cadence. Importing or saving a contact therefore records
identity without silently declaring that the relationship needs nurturing. The
owner can promote the relationship deliberately, and an active Plan can still
make a lowest-level contact actionable. Existing closeness values are left
alone; only new writes use the Acquaintance default. As a defense for older
data, untouched bulk-imported People with no past Interaction and no active
Plan are excluded from attention surfaces.
...
```mermaid
flowchart TD
  ImportHub["/import chooser"] --> PeopleImport["/import/persons"]
  ImportHub --> InteractionImport["/import/interactions"]
  LegacyConversation["/import/conversations"] --> InteractionImport
  PeopleFile["Spreadsheet, vCard, or CSV people file"] --> PeopleImport
...
Plain English: import is a bulk way to turn source material into structured People, Events, and Interactions. `/import` is the chooser, `/import/persons` handles arbitrary `.xlsx` people spreadsheets, vCard/CSV people files, and Google Contacts from the connected Gmail account. Spreadsheet import finds person-shaped tables anywhere in a workbook, maps familiar contact columns, and keeps other row-specific fields as editable Notes. The user still reviews every candidate and chooses create, update, or skip before anything is saved. Ambiguous non-person sheets are reported but are not silently turned into Events, Places, or Items. `/import/interactions` handles Gmail Mail, transcripts, notes, and message exports. `/import/conversations` remains as a redirect for older links.
...
Each connected calendar has an owner-attendance default. Presence is assumed unless that calendar opts out — typically a shared family calendar whose events are not "mine" unless I say so. A per-occurrence Going / Not going mark on the Plan overrides the calendar default and is declared intent, not an Interaction. Did go / Didn't on a past Plan is the existing confirmation path: it creates the Event and attendee Interactions, or records that it did not happen. The gap between declared attendance and that confirmation is the tension layer (I said I would go and didn't; I said I wouldn't and showed up).
...
erDiagram
  ...
  LifeModelSnapshot ||--o{ LifeModelAnalysisRun : produced_by
  LifeModelClaim ||--o{ LifeModelClaimFeedback : receives
  Note ||--o{ LifeModelClaimFeedback : grounds_correction
```
...
- **LifeModelSnapshot, LifeModelClaim, LifeModelAnalysisRun, LifeModelClaimFeedback**: whole-life synthesis (`packages/intelligence`, formerly `packages/theory` — Theory of Person is unchanged, this is the workspace-scoped sibling). An explicit Home command makes a billed Vercel AI Gateway call over bounded evidence from all eight primitives plus Interactions and prior human feedback; the run receipt records model, status, tokens, and cost. The resulting snapshot is append-only and holds grounded `observed`/`inferred`/`declared` claims, while deterministic `tension` claims still come from `packages/alignment` at confidence 1. Correction and dismissal append feedback rather than rewriting model output. A correction also creates an `intelligence_correction` Note, making the user's declared truth part of the graph and the highest-authority evidence for later readings.
...
- Person naming: first name is required, while last name is optional across the core create/update command, Add Person, Edit Person, Inbox person creation, API clients, and imports. A person who uses one name is stored honestly with an empty last-name value; the UI never asks for a dash or invented surname.
