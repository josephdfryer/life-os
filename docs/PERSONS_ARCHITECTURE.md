...
- AI-backed synthesis for the whole-life model: `synthesizeLifeModel`'s `tension` claims are real (deterministic, from `packages/alignment`); `observed`/`inferred`/`declared` are still honest Unknown placeholders, unlike Theory of Person's now-real synthesis. Deliberately not done yet: `apps/home/app/intelligence/page.tsx` calls `synthesizeLifeModel` directly as a live fallback preview whenever no snapshot exists — making that function itself AI-backed would fire a real, billed AI call on every such page view. Needs an explicit Home-side regenerate trigger first (`regenerateLifeModel` currently has zero callers anywhere) — a cross-track coordination point.
