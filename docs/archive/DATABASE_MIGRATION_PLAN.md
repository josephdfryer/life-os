...
| P8     | **Runbook written, pending user** | Step-by-step: **[`DATABASE_CUTOVER_RUNBOOK.md`](DATABASE_CUTOVER_RUNBOOK.md)**. Tooling built and tested: `scripts/db/cutover.ts preflight` (readiness gate) and `verify` (independent post-ETL Neon-vs-Turso reconciliation — 95-model recount, Person deep-compare, FK validation, orphan check); `scripts/db/purge-turso-test-residue.ts` (the 83-row cleanup); reuses `scripts/sync-vercel-env.ts` for the 8-project env flip. Writers to freeze are enumerated (2 GitHub workflows, 2 Vercel crons, 12 LaunchAgents). **DateTime note:** `verify` initially flagged a 7h shift on `Person.createdAt` — traced to raw node-postgres parsing `timestamp without time zone` in the process's local TZ; through the real app path (`@life-os/db` → `@prisma/adapter-pg`) the instant round-trips **exactly** (`2026-07-06T16:43:38.632Z` in Turso → same in Neon). Not a data bug; `verify` now pins pg's type parser to UTC. Schema stays on Prisma's default `timestamp(3)`; moving to `@db.Timestamptz` is optional future hardening, not needed. |
